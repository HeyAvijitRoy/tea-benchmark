...
6. [Step 3 — Figure Generation (`generate_tfr_figure.py`)](#6-step-3--figure-generation)
7. [Output File Reference](#7-output-file-reference)
8. [Interpreting the Results](#8-interpreting-the-results)
9. [Run Logging (`log_run.py`)](#9-run-logging)
...
tea-benchmark/
├── data/
│   ├── tea_corpus.csv            # Raw 120-item corpus (source of truth)
│   ├── tea_corpus_flagged.csv    # Generated by Step 1
│   └── quality_report.txt        # Generated by Step 1
├── results/                      # Generated by Step 2 (created automatically)
│   ├── raw_token_counts.csv
│   ├── tfr_by_item.csv
│   ├── summary_stats.csv
│   ├── acm_table.csv
│   └── ecw_table.csv
├── figures/                      # Generated by Step 3 (created automatically)
...
│   ├── generate_tfr_figure.py    # Step 3: Publication figure generator
│   └── log_run.py                # Run logger — appends entries to Notes.md
├── requirements.txt
├── README.md
├── RUNBOOK.md                    # This file
└── Notes.md                      # Auto-updated run log
```
...
For each of the 120 corpus items, the script computes the ratio of Bengali Unicode characters (U+0980–U+09FF) to total alphabetic characters in the Bengali translation column. Items are classified into three quality tiers:
...
── Overall Distribution ──────────────────────────────
  clean               :  62 items  (51.7%)
  mixed               :  50 items  (41.7%)
  english_retained    :   8 items  ( 6.7%)
  TOTAL               : 120 items

── Ratio Statistics ─────────────────────────────────
  Mean bn_ratio  : 0.746
  Median bn_ratio: 0.855
  ...
...
1. Loads `tea_corpus_flagged.csv` (120 items).
...
3. Tokenizes all six language columns (`English`, `Bengali`, `Hindi`, `Arabic`, `Tamil`, `Yoruba`) for every item under every tokenizer — 2,160 combinations total. Missing/empty cells produce `NaN` rather than errors.
...
| Tokenizing 120 items × 6 languages × 3 tokenizers | 30–120 s |
...
Bengali                   1.5616              4.4969              4.4416
Hindi                     1.7190              4.8646              5.1957
Arabic                    1.4358              1.6957              3.8587
Tamil                     2.0942              6.5546              6.5681
Yoruba                    2.3725              3.1755              3.3340
========================================================================
...
ACM TABLE (GPT-4o / tiktoken_o200k):
  ...
  Bengali           1.5616      1.5616                      0.0717
  Hindi             1.7190      1.7190                      0.0918
  Arabic            1.4358      1.4358                      0.0556
  Tamil             2.0942      2.0942                      0.1396
  Yoruba            2.3725      2.3725                      0.1752

EFFECTIVE CONTEXT WINDOW TABLE (GPT-4o, nominal=128k):
  Language        Mean TFR    ECW (tokens)   ECW % of 128k
  -----------  ----------  --------------  --------------
  English           1.0000         128,000         100.00%
  Bengali           1.5616          81,967          64.04%
  Hindi             1.7190          74,461          58.17%
  Arabic            1.4358          89,148          69.65%
  Tamil             2.0942          61,121          47.75%
  Yoruba            2.3725          53,951          42.15%

Output files written:
  results/raw_token_counts.csv               2160 rows
  results/tfr_by_item.csv                    2160 rows
...
**Input:** hardcoded TFR and ECW values (derived from `results/acm_table.csv` and `results/ecw_table.csv`)
**Outputs:** four files under `figures/`

### What it does

Generates two publication-quality figures formatted for an IJCAI workshop paper:

- **Figure 1 — TFR grouped bar chart:** three bars per language (GPT-4o, Qwen2.5-7B, Mistral-7B), dashed English baseline at y = 1.0, value labels on each bar, colorblind-safe palette (Wong 2011).
- **Figure 2 — ECW bar chart:** effective token capacity per language under GPT-4o tokenization, with the nominal 128k window shown as a dashed reference line, dual labels showing token count and % of nominal.

Both figures are saved as vector PDF (for LaTeX/Overleaf submission) and 300 DPI PNG (for preview).
...
```
Saved: figures/tfr_by_language_tokenizer.pdf
Saved: figures/tfr_by_language_tokenizer.png
Saved: figures/ecw_by_language.pdf
Saved: figures/ecw_by_language.png
Notes.md updated (2026-05-10 — generate_tfr_figure.py)
```
...
| File | Description |
|---|---|
| `figures/tfr_by_language_tokenizer.pdf` | TFR figure — vector PDF for paper submission |
| `figures/tfr_by_language_tokenizer.png` | TFR figure — 300 DPI raster preview |
| `figures/ecw_by_language.pdf` | ECW figure — vector PDF for paper submission |
| `figures/ecw_by_language.png` | ECW figure — 300 DPI raster preview |
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/tfr_by_language_tokenizer.pdf}
  \caption{Token Fertility Ratio by language and tokenizer (TEA Benchmark, $n=120$).
           Dashed line marks the English baseline (TFR = 1.0).}
  \label{fig:tfr}
\end{figure*}

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/ecw_by_language.pdf}
  \caption{Effective context window under GPT-4o tokenization.
           Nominal window = 128{,}000 tokens.}
  \label{fig:ecw}
\end{figure}
```
...
All files are written to `results/` with UTF-8 encoding. Row counts assume all 120 items with no missing data.

---

### `raw_token_counts.csv` — 2,160 rows
...
**Row count derivation:** 120 items × 6 languages × 3 tokenizers = 2,160.

---

### `tfr_by_item.csv` — 2,160 rows
...
- `all` — all 120 items included for every language.
- `clean_only` — for Bengali, only items where `bn_quality == "clean"` are included (sensitivity check for translation quality). All other languages are unaffected and retain all 120 items.
...
|---|---|---|
| `language` | string | Language |
| `mean_tfr_gpt4o` | float | Mean TFR under `tiktoken_o200k` across all 120 items |
...
**Interpretation:** A Tamil user fitting a document into GPT-4o's 128k window effectively has access to only ~61,121 tokens of content capacity — 47.8% of what an English user receives for the same nominal price.
...
Fifty-eight Bengali items (50 mixed + 8 english_retained, 48.3% combined) were classified below `clean` due to code-switching (retained Python identifiers like `TypeError`, `NoneType`). The `clean_only` subset excludes these from Bengali TFR computation to check whether they inflate or deflate the overall Bengali penalty. Compare `mean_tfr` values between `subset=all` and `subset=clean_only` in `summary_stats.csv` to assess the sensitivity.

---

## 9. Run Logging

**Script:** `scripts/log_run.py`

Every pipeline script (`01_quality_check.py`, `02_tokenize.py`, `generate_tfr_figure.py`) calls `log_run.append_run_entry()` as its final step, automatically appending a dated entry to `Notes.md` with key stats. No manual action is needed.

You can also call the logger directly from the command line:

```bash
# Basic — just records the script name and date
python scripts/log_run.py --script 02_tokenize.py

# With a note
python scripts/log_run.py --script 02_tokenize.py --note "re-ran after corpus update"

# With structured stats
python scripts/log_run.py --script 02_tokenize.py --stats items=70 languages=6 tokenizers=3
```

Each entry written to `Notes.md` looks like:

```markdown
## 2026-05-10 — `02_tokenize.py` run
